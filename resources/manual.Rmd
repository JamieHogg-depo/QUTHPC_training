...
In practice, most of the time you're use of the HPC head node will be to monitor your jobs, so ensure your accustumed to the `qstat` command. 
...
Unlike the output file, the error file is useful for diagnosing errors produced by R during the HPC job. This is becuase the output file provides some of the output from `R` like you would see in `Rstudio` on your local machine. 
