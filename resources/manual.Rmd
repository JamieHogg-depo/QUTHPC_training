...
By running `qsub <sub_file>`, we tell the PBS system to read the file for both the computational resources we required and the code we wish to run. When running a single `.sub` file, the PBS will acknowledge your request for a job by printing a unique PBS number. In the example above this is `5059221.pbs`. If you do not see a unique PBS number, then there is something wrong with your `.sub` file.    
...
In the error file you might see this line.
...
## Incorrect time allocated

Sometimes you might underestimate how long your model will take to run, resulting in the HPC stopping your job before it's done. 

In the error file you will see something like 
